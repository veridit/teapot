...
pub fn save_latex(
    ...
    if body {
        for z in z1..=z2 {
            ...
            write_latex_table(&mut fp, sheet, x1, y1, x2, y2, z, &mut count)?;
        }
    } else {
        let mut fp = std::fs::File::create(name)?;
        writeln!(fp, "\\documentclass{{article}}\n\\usepackage{{longtable}}\n\\begin{{document}}\n\\setlongtables\n\\vfill")?;
        for z in z1..=z2 {
            if z > z1 {
                writeln!(fp, "\\clearpage\n\\vfill")?;
            }
            write_latex_table(&mut fp, sheet, x1, y1, x2, y2, z, &mut count)?;
            writeln!(fp, "\\vfill")?;
        }
        writeln!(fp, "\\end{{document}}")?;
    }
...
fn write_latex_table(
    ...
    for y in y1..=y2 {
        ...
        while x <= x2 {
            ...
            let align_char = match get_adjust(sheet, x, y, z) {
                Adjust::Left | Adjust::AutoAdjust => 'l',
                Adjust::Right => 'r',
                Adjust::Center => 'c',
            };
            write!(fp, "\\multicolumn{{{}}}{{{}}}{{", multicols, align_char)?;

            let val = format_value(sheet, x, y, z);
            if is_transparent(sheet, x, y, z) {
                write!(fp, "{}", val)?;
            } else {
                write!(fp, "{}", escape_latex(&val))?;
            }
            write!(fp, "}}")?;
